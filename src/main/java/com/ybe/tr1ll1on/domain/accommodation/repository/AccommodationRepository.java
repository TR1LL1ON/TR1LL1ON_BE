...
/*
 * 1. Accommodation 엔티티를 가져올 때, 해당 엔터티와 연관된 Product 엔터티 리스트는 데이터베이스에서 로딩되지 않는다. (Fetch Type = LAZY)
 * 2. 이후 접근할 때마다 데이터베이스에서 추가적인 쿼리가 실행되어 각각의 Product 엔터티를 가져오게 된다.
 * 3. 이로 인해 총 N+1 번의 쿼리가 실행되는 문제가 발생한다. (Accommodation 엔터티를 가져오는 1번의 쿼리 + Product 엔터티 리스트를 가져오는 N번의 쿼리)
 */

/*
 * fetch join을 사용하게 되면 Product 엔터티 리스트를 함께 로딩하므로,
 * Accommodation 엔터티를 가져올 때 추가적인 쿼리 없이 한 번의 쿼리로 필요한 데이터를 모두 가져올 수 있다.
 */
